...
export async function generatePostAxiosCode(swaggerFilePath: string, args?: {
    Bearer?: string,
    apiKey?: string,
    oAuth2?: string
}) {
        ...
        if (api.paths) {
            for (const [path, pathItem] of Object.entries(api.paths)) {
                ...
                if (operation) {
                    ...
                    const example = await generateRequestBodyExample(swaggerFilePath, path, 'post');
...
export async function generatePostAxiosCodeFromSwagger(swaggerFilePath: string, args?: {
    Bearer?: string,
    apiKey?: string,
    oAuth2?: string
}) {

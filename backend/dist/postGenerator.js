...
exports.generatePostAxiosCodeFromSwagger = exports.generatePostAxiosCode = void 0;
...
function generatePostAxiosCode(swaggerFilePath, args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            let fileContent = 'import { sendPostRequest } from "../src/sendRequests";\n\n';
            if (api.paths) {
                for (const [path, pathItem] of Object.entries(api.paths)) {
                    ...
                    if (operation) {
                        ...
                        const example = yield (0, generateValue_1.generateRequestBodyExample)(swaggerFilePath, path, 'post');
                        fileContent += `sendPostRequest('POST', '${fullUrl}', ${JSON.stringify(headers)}, ${JSON.stringify(example)});\n`;
                    }
                }
                return fileContent;
            }
...
exports.generatePostAxiosCode = generatePostAxiosCode;
// async function main() {
//     try {
//         const cliArgs = await cli();
//         await generatePostAxiosCode(cliArgs.file);
//     } catch (error: any) {
//         console.error(`Error in POST generator: ${error.message}`);
//     }
// }
// main();
function generatePostAxiosCodeFromSwagger(swaggerFilePath, args) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        const api = yield SwaggerParser.parse(swaggerFilePath);
        const securitySchemes = yield (0, auth_1.getSecuritySchemes)(api);
        ...
        for (const [path, pathItem] of Object.entries(api.paths)) {
            ...
            if (operation) {
                ...
                const fullUrl = `${fullPath}${queryString}`;
                const example = yield (0, generateValue_1.generateRequestBodyExample)(swaggerFilePath, path, 'post');
                requests.push(`sendPostRequest('POST', '${fullUrl}', ${JSON.stringify(headers)}, ${JSON.stringify(example)});`);
            }
        }
        return requests;
    });
}
exports.generatePostAxiosCodeFromSwagger = generatePostAxiosCodeFromSwagger;
